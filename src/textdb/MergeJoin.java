...
public class MergeJoin extends Operator {
	...
	public Tuple next() throws IOException {
		Tuple tupleLeft = input[0].next();
		Tuple tupleRight = input[1].next();
		if (tupleLeft == null || tupleRight == null) {
			return null;
		}	
		String left_key = String.valueOf(tupleLeft.getValues()[0]);
		String right_key = String.valueOf(tupleRight.getValues()[0]);
		while (!tupleLeft.getValues()[0].equals(tupleRight.getValues()[0])) {
			left_key = String.valueOf(tupleLeft.getValues()[0]);
			right_key = String.valueOf(tupleRight.getValues()[0]);
			if (left_key.compareTo(right_key) > 0) {
				tupleRight = input[1].next();			
			} else {
				tupleLeft = input[0].next();
			}
			if (tupleLeft == null || tupleRight == null) return null;	
		}
